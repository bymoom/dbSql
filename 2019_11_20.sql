--특정 테이블의 컬럼 조회
--1. DESC 테이블명
--2. SELECT * FROM user_tab_colums

--prod 테이블의 컬럼조회
DESC prod;

--(prod 테이블에서 널? 컬럼 설명: Not null 값을 넣어야한다, 빈칸 값을 안 넣어도 된다)
...
--별칭 : 기본 컬럼명이나 연산을 통해 생성된 가상 컬럼에 임의의 컬럼이름을 부여
--      col : sxpress [AS] 별칭명(AS 생략가능:reg_dt reg_date)
SELECT userid, usernm, reg_dt reg_date, reg_dt+5 AS afterSday
FROM users;

-- 숫자 상수, 문자열 상수 (오라클에서는 문자열을 ''로 표현)
...
--(단 prod_id -> id, prod_name 으로 별칭을 지정)
SELECT prod_id, prod_name
FROM prod;

--lprod 테이블에서 lprod_gu, 1prod_nm 두 컬럼을 조회하는 쿼리를 작성하시오.
...
--(WHERE = low절) : 조건이 일치하는 행만 조회하기 위해 사용
--                    행에 대한 조회 기준을 작성
...
--사원입사일자(hiredate)가 1981년 1월 1일 이후인 사원 정보 조회
--문자열-->날짜 타입으로 변경 TO_DATE('날짜문자열', '날짜문자열포맷')
--한국 날짜 표현 : 연-월-일
--미국 날짜 표현 : 일-월-연 (01-01-2020)
